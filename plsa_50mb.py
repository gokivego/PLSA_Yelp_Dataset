...
file1 = codecs.open('dataset.txt', 'r','utf-8')
doc_set = [document.strip() for document in file1] 
doc_set = filter(None,doc_set)
file1.close
...
for i in doc_set:
    ...
    raw=raw.translate(translate_table)
    raw=re.sub(r'\d+','',raw)
...
old_corpus = [dictionary.doc2bow(text) for text in texts]
corpus=[{} for i in range(len(old_corpus))]

for i in range(len(corpus)):
    for x,y in old_corpus[i]:
        corpus[i][x]=y
...
# The following code has been forked from github https://github.com/isnowfy/plsa/blob/master/plsa.py

class Plsa:

    def __init__(self, corpus, topics=30):
        self.topics = topics
        self.corpus = corpus
    ...
    def inference(self, doc, max_iter=1000):
